...
，因为默认参数L也是一个变量（有自己的内存地址），它指向对象[1,2,3]，每次调用该函数，\
如果改变了L的内容，则下次调用时，默认参数的内容就变了，不再是函数定义时的[1,2,3]了"

"个人理解：当初创建时l就有了自己的内存地址，当没有参数传递时l就会引用自己的内存地址，当有新的参数\
    传递时，l元内存地址被覆盖，就会使用新的内存地址。"
...
'@@@@@@@@@我们把对象比作塑料盒子(实例化后)。类（class）则像是制作盒子用的模具（为实例化）'
